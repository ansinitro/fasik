...
import config
from src.data_collection import collect, features, scraper
...
def run_module_function(module_func, description):
    """Run a module function and handle errors"""
...
def create_directories():
    ...
    directories = ['data', 'models', 'results', 'src/web/templates']
...
def run_full_pipeline():
    ...
    pipeline_steps = [
        ...
        },
        {
            ...
            "required_files": [config.PRO_DATA_CSV],
            "output_files": [config.PRO_IDS_JSON, config.HIGH_LEVEL_IDS_JSON, config.NORMAL_IDS_JSON],
        ...
        },
        {
            ...
            "required_files": [config.PRO_IDS_JSON, config.HIGH_LEVEL_IDS_JSON, config.NORMAL_IDS_JSON],
            "output_files": [config.DATASET_CSV],
            "estimated_time": "30-60 minutes ⏰ (API rate limited)"
        },
        {
            "num": 5,
            "func": preprocess.main,
            "title": "PREPROCESS DATA",
            "description": "Feature engineering and data splitting",
            "required_files": [config.DATASET_CSV],
            "output_files": [config.X_TRAIN, config.Y_TRAIN],
        ...
        },
        {
            ...
            "required_files": [config.X_TRAIN],
            "output_files": [config.XGBOOST_MODEL, config.BEST_MODEL_TXT],
            "estimated_time": "5-10 minutes"
        },
        {
            "num": 7,
            ...
            "required_files": [config.DATASET_CSV],
            "output_files": [config.CLASS_STATISTICS_JSON],
    ...
    for step in pipeline_steps:
        ...
        # Run the script
        success = run_module_function(step["func"], step["description"])
    ...
    print(f"   {Colors.CYAN}python -m src.web.app{Colors.END}")
...
def run_single_step(step_number):
    ...
    steps = {
        1: (scraper.save_player_data_to_csv, "Collect Pro Players"),
        2: (collect.main, "Collect Player IDs"),
        3: (features.main, "Extract Features"),
        4: (preprocess.main, "Preprocess Data"),
        5: (train.main, "Train Models"),
        6: (stats.main, "Calculate Statistics"),
        7: (None, "Run Web Application") # Handled separately
    }
    ...
    func, description = steps[step_number]
    ...
    if step_number == 7:
        ...
        return run_module_function(func, description)
...
def check_system_status():
    ...
    files_to_check = {
        "Pro Players Data": config.PRO_DATA_CSV,
        "Pro Player IDs": config.PRO_IDS_JSON,
        "High-Level Player IDs": config.HIGH_LEVEL_IDS_JSON,
        "Normal Player IDs": config.NORMAL_IDS_JSON,
        "Feature Dataset": config.DATASET_CSV,
        "Training Data": config.X_TRAIN,
        "Trained Models": config.XGBOOST_MODEL,
        "Class Statistics": config.CLASS_STATISTICS_JSON
    }
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "full":
            ...
            print_info("Usage: python pipeline.py [full|1-7|status|app]")
    else:
        # Interactive mode
        while True:
            choice = show_menu()
