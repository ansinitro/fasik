import os
import sys

# Add parent directory to sys.path to allow importing config and src
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import config

def verify():
    print("Testing data paths...")
    
    # 1. Check Data Directory
    if os.path.exists(config.DATA_DIR):
        print(f"✅ Data directory exists: {config.DATA_DIR}")
    else:
        print(f"❌ Data directory missing: {config.DATA_DIR}")

    # 2. Check Data Files (if they exist)
    files_to_check = [
        ("PRO_DATA_CSV", config.PRO_DATA_CSV),
        ("PRO_IDS_JSON", config.PRO_IDS_JSON),
        ("HIGH_LEVEL_IDS_JSON", config.HIGH_LEVEL_IDS_JSON),
        ("NORMAL_IDS_JSON", config.NORMAL_IDS_JSON),
        ("DATASET_CSV", config.DATASET_CSV)
    ]

    for name, path in files_to_check:
        if os.path.exists(path):
            print(f"✅ {name} found at: {path}")
        else:
            print(f"⚠️  {name} not found (might not be generated yet)")

    print("\nTesting imports...")
    try:
        from src.data_collection import scraper
        print("✅ src.data_collection.scraper imported successfully")
    except Exception as e:
        print(f"❌ src.data_collection.scraper import failed: {e}")

    try:
        from src.data_collection import collect
        print("✅ src.data_collection.collect imported successfully")
    except Exception as e:
        print(f"❌ src.data_collection.collect import failed: {e}")

    try:
        from src.data_collection import features
        print("✅ src.data_collection.features imported successfully")
    except Exception as e:
        print(f"❌ src.data_collection.features import failed: {e}")

if __name__ == "__main__":
    verify()
